...
func (impl *Implementation) CreateWithOptions(ctx context.Context, req *desc.CreateWithOptionsRequest) (*desc.CreateWithOptionsResponse, error) {
	newQuestionWithOptions := converter.NewQuestionWithOptionsFromGRPC(req)
	id, err := impl.questionFormUpdaterService.CreateWithOptions(ctx, newQuestionWithOptions)
	if err != nil {
		return nil, err
	}
